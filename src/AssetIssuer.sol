...
contract AssetIssuer is Initializable, AssetController, IAssetIssuer {
    ...
    function getIssueAmountRange(
        ...
        require(
            _minAmounts.contains(assetID) && _maxAmounts.contains(assetID),
            "issue amount range not set"
        );
        return
            Range({
    ...
    function setIssueAmountRange(
        ...
        require(
            issueAmountRange.min <= issueAmountRange.max &&
                issueAmountRange.max > 0 &&
                issueAmountRange.min > 0,
            "wrong range"
        );
        _minAmounts.set(assetID, issueAmountRange.min);
        _maxAmounts.set(assetID, issueAmountRange.max);
        emit SetIssueAmountRange(
            assetID,
            _minAmounts.get(assetID),
            _maxAmounts.get(assetID)
        );
    }
    ...
    function addMintRequest(
        ...
        require(
            _participants[assetID].contains(msg.sender),
            "msg sender not a participant"
        );
        require(
            _minAmounts.contains(assetID) && _maxAmounts.contains(assetID),
            "issue amount range not set"
        ...
        require(
            keccak256(abi.encode(assetToken.getTokenset())) ==
                keccak256(abi.encode(order.outTokenset)),
            "tokenset not match"
        );
        require(
            order.outAmount >= _minAmounts.get(assetID) &&
                order.outAmount <= _maxAmounts.get(assetID),
            "mint amount not in range"
        );
        Token[] memory inTokenset = order.inTokenset;
        uint256 issueFee = _issueFees.get(assetID);
        for (uint i = 0; i < inTokenset.length; i++) {
            require(
                bytes32(bytes(inTokenset[i].chain)) ==
            ...
            require(
                inToken.balanceOf(msg.sender) >= transferAmount,
                "not enough balance"
            );
            require(
                inToken.allowance(msg.sender, address(this)) >= transferAmount,
    ...
    function rejectMintRequest(
        ...
        for (uint i = 0; i < inTokenset.length; i++) {
            ...
            uint transferAmount = inTokenAmount + feeTokenAmount;
            require(
                inToken.balanceOf(address(this)) >= transferAmount,
                "not enough balance"
            );
    ...
    function confirmMintRequest(
        ...
        for (uint i = 0; i < inTokenset.length; i++) {
            require(
                bytes32(bytes(inTokenset[i].chain)) == bytes32(bytes(chain)),
                "chain not match"
            );
            address tokenAddress = Utils.stringToAddress(inTokenset[i].addr);
            IERC20 inToken = IERC20(tokenAddress);
            ...
            uint feeTokenAmount = (inTokenAmount * mintRequest.issueFee) /
                10 ** feeDecimals;
            require(
                inToken.balanceOf(address(this)) >= feeTokenAmount,
                "not enough balance"
            );
    ...
    function addRedeemRequest(
        ...
        ISwap swap = ISwap(swapAddress);
        require(
            assetToken.hasRole(assetToken.ISSUER_ROLE(), address(this)),
            "not a issuer"
        );
        require(assetToken.feeCollected(), "has fee not collect");
        require(assetToken.rebalancing() == false, "is rebalancing");
        ...
        require(
            keccak256(abi.encode(assetToken.getTokenset())) ==
                keccak256(abi.encode(order.inTokenset)),
            "tokenset not match"
        );
        require(
            order.inAmount >= _minAmounts.get(assetID) &&
                order.inAmount <= _maxAmounts.get(assetID),
            "redeem amount not in range"
        );
        require(
            assetToken.balanceOf(msg.sender) >= order.inAmount,
            "not enough asset token balance"
        );
        require(
            assetToken.allowance(msg.sender, address(this)) >= order.inAmount,
            "not enough asset token allowance"
        );
        Token[] memory outTokenset = order.outTokenset;
        for (uint i = 0; i < outTokenset.length; i++) {
            require(
                bytes32(bytes(outTokenset[i].chain)) ==
                    bytes32(bytes(factory.chain())),
                "chain not match"
            );
            require(
                Utils.stringToAddress(order.outAddressList[i]) == address(this),
                "out address not valid"
            );
        }
    ...
    function rejectRedeemRequest(uint nonce) external onlyOwner {
        ...
        require(
            redeemRequest.status == RequestStatus.PENDING,
            "redeem request is not pending"
        );
        ISwap swap = ISwap(redeemRequest.swapAddress);
        SwapRequest memory swapRequest = swap.getSwapRequest(
            redeemRequest.orderHash
        );
        require(
            swapRequest.status == SwapRequestStatus.REJECTED,
            "swap request is not rejected"
        );
        IAssetToken assetToken = IAssetToken(redeemRequest.assetTokenAddress);
        require(
            assetToken.balanceOf(address(this)) >= redeemRequest.amount,
            "not enough asset token to transfer"
        );
    ...
    function confirmRedeemRequest(
        ...
        require(
            assetToken.balanceOf(address(this)) >= redeemRequest.amount,
            "not enough asset token to burn"
        );
        Order memory order = orderInfo.order;
        Token[] memory outTokenset = order.outTokenset;
        ...
        for (uint i = 0; i < outTokenset.length; i++) {
            ...
            require(
                outToken.balanceOf(address(this)) >= outTokenAmount,
                "not enough balance"
            );
    ...
    function withdraw(address[] memory tokenAddresses) external onlyOwner {
        ...
        for (uint i = 0; i < assetIDs.length; i++) {
            IAssetToken assetToken = IAssetToken(
                factory.assetTokens(assetIDs[i])
            );
